...
function promote_f(f::F, ::Val{specialize}, u0, p, t) where {F, specialize}
    ...
    return f = if f isa ODEFunction && isinplace(f) && !(f.f isa AbstractSciMLOperator) &&
        ...
        # Wrap the initialization NonlinearFunction if present, so the
...
# Wrap the NonlinearFunction inside OverrideInitData.initializeprob so that
# the NonlinearSolve dispatch sees a type-erased function and can reuse
# compiled code across different models. Also wraps initializeprobmap and
# initializeprobpmap so their types are erased in the OverrideInitData.
function _wrap_initialization_data(f)
    ...
    if SciMLBase.isinplace(nlf) && !(nlf.f isa FunctionWrappersWrappers.FunctionWrappersWrapper)
        ...
        if init_u0 !== nothing
            # nlf!(residual, u, p) -> Nothing
            wrapped_nlf_f = wrapfun_jac_iip(nlf.f, (similar(init_u0), init_u0, init_p))
            # Build a type-erased NonlinearFunction: F is the FWW type (same across models),
            # function-typed fields use Nothing (guaranteed nothing for init functions),
            # observed uses Any (may differ per model).
